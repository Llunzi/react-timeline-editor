...
export const RowDnd = React.forwardRef<RowRndApi, RowRndProps>(
  (
    ...
    useImperativeHandle(ref, () => ({
      ...
      updateHeight: (height) => handleUpdateHeight(height, false),
    ...
    useEffect(() => {
      const target = interactable.current.target as HTMLElement;
      handleUpdateHeight(typeof height === 'undefined' ? target.offsetHeight : height, false);
    }, [height]);
    useEffect(() => {
    ...
    const handleUpdateHeight = (height: number, reset = true) => {
      if (!interactable.current || !interactable.current.target) return;
      reset && (deltaY.current = 0);
      const target = interactable.current.target as HTMLElement;
      target.style.height = `${height}px`;
      Object.assign(target.dataset, { height });
    };
    const handleGetTop = () => {
      const target = interactable.current.target as HTMLElement;
    ...
    const handleGetHeight = () => {
      const target = interactable.current.target as HTMLElement;
      return parseFloat(target?.dataset?.height || '0');
    };
    //#endregion
    ...
    const move = (param: { preLeft: number; preWidth: number; preTop: number; preHeight: number; scrollDelta?: number }) => {
      const { preLeft, preWidth, preTop, preHeight, scrollDelta } = param;
      ...
      if (Math.abs(deltaX.current) >= distance) {
        ...
        curLeft = preLeft + count * distance;
      ...
      // 处理Y轴移动
      if (Math.abs(deltaY.current) >= grid) {
        const count = parseInt(deltaY.current / grid + '');
        curTop = preTop + count * grid;
        deltaY.current = deltaY.current % grid;

        // 控制Y轴bounds
        if (bounds.top !== undefined && curTop < bounds.top) curTop = bounds.top;
        if (bounds.bottom !== undefined && curTop + preHeight > bounds.bottom) curTop = bounds.bottom - preHeight;
      }

      // 如果X轴或Y轴有变化,触发回调
      if (curLeft !== preLeft || curTop !== preTop) {
        if (onDrag) {
          const ret = onDrag(
            {
              lastLeft: preLeft,
